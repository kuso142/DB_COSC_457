...
public class VendorPanel extends JPanel {
    ...
    private JPanel buildTopBar() {
        ...
        refreshVendorCombo();
        cbVendor.addActionListener(e -> { loadMenu(); loadOrders(); });
        ...
        p.add(btnAddVendor); p.add(btnDelVendor);
        return p;
    }
    ...
    private JSplitPane buildSplitCenter() {
        ...
        menuButtons.add(btnAdd); menuButtons.add(btnEdit); menuButtons.add(btnDelete);
        ...
        ordersButtons.add(btnRefresh); ordersButtons.add(btnMarkDone);
    ...
    private void refreshVendorCombo() {
        ...
        try { for (Vendor v : vendorDAO.getAllVendors()) cbVendor.addItem(v); }
    ...
    private void addVendor() {
        ...
        JTextField tfCuisine = new JTextField(12);
        ...
        form.add(new JLabel("Cuisine Type:")); form.add(tfCuisine);
        ...
        if (tfName.getText().isBlank() || tfAddr.getText().isBlank() || tfPhone.getText().isBlank()) {
            JOptionPane.showMessageDialog(this, "Name, address and phone are required."); return;
        }
        try {
            vendorDAO.insert(new Vendor(0, tfName.getText().trim(), tfAddr.getText().trim(),
                                        tfPhone.getText().trim(), tfCuisine.getText().trim()));
            refreshVendorCombo();
        } catch (SQLException e) { showError(e); }
    }
    ...
    private void addMenuItem() {
        Vendor v = (Vendor) cbVendor.getSelectedItem();
        if (v == null) { JOptionPane.showMessageDialog(this, "Select a restaurant first."); return; }
        ...
        JPanel form = new JPanel(new GridLayout(0, 2, 4, 4));
        form.add(new JLabel("Item Name:"));   form.add(tfName);
        ...
        try {
            ...
            if (price <= 0) { JOptionPane.showMessageDialog(this, "Price must be > 0."); return; }
            menuItemDAO.insert(new MenuItem(0, v.getVendorId(), tfName.getText().trim(), price, tfDesc.getText().trim()));
            loadMenu();
    ...
    private void editMenuItem() {
        ...
        try {
            ...
            if (p <= 0) { JOptionPane.showMessageDialog(this, "Price must be > 0."); return; }
            Vendor v = (Vendor) cbVendor.getSelectedItem();
            menuItemDAO.update(new MenuItem(itemId, v.getVendorId(), tfName.getText().trim(), p, tfDesc.getText().trim()));
            loadMenu();
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Invalid price.");
        } catch (SQLException e) { showError(e); }
    ...
    private void deleteMenuItem() {
        ...
        if (confirm != JOptionPane.YES_OPTION) return;
        try { menuItemDAO.delete(itemId); loadMenu(); }
        catch (SQLException e) { showError(e); }
    }
